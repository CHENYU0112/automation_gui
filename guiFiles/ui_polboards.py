...
class PolBoards(ttk.Frame):
    ...
    modelDict = {
        '3DiPOL': dipolModel,
        'Ares': aresModel,
        'Atlantic': atlanticModel,
        'Auroville': aurovilleModel,
        'Baileys': baileysModel,
        'Coronado': coronadoModel,
        'Easton': eastonModel,
        'Horseneck': horseneckModel,
        'Malibu': malibuModel,
        'Miramar': miramarModel,
        'Newport': newportModel,
        'Redondo': redondoModel,
        'Custom': customModel,
    }
    ...
    def __init__(self, parent):
        super(PolBoards, self).__init__(parent)
        ...
        helpMsg = '''Help - POL Board (family, model). Used to name the folder where data from test is being saved.'''
        imgArgLst = [
            helpFrame,
            helpMsg,
        ]
        ...
        PolBoards.varPolBoardFamily = StringVar()
        ...
        self.varPolBoardFamily.set(ConfigMgr.instr['polBoardFamily'])
        ...
        PolBoards.varPolBoardModel = StringVar()
        ...
        self.varPolBoardModel.set(ConfigMgr.instr['polBoardModel'])
        ...
        PolBoards.varSiliconRev = StringVar()
        entry = ttk.Entry(
            self,
        ...
        PolBoards.varSiliconRev.set(ConfigMgr.instr['polBoardSilicRev'])
    ...
    def pol_board_family(self, content):
        if self.varPolBoardFamily.get() == 'Custom':
            ...
            self.entryFamily.grid_remove()
            self.entryModel.grid_remove()
            ConfigMgr.instr['polBoardFamily'] = self.varPolBoardFamily.get()
            self.modelList = self.modelDict[self.varPolBoardFamily.get()]
            self.modelDropMenu.grid_remove()
            self.modelDropMenu = ttk.OptionMenu(
                self,
    ...
    def pol_board_model(self, content):
        ConfigMgr.instr['polBoardModel'] = self.varPolBoardModel.get()
        return True

    def silicon_rev(self):
        ConfigMgr.instr['polBoardSilicRev'] = self.varSiliconRev.get()
        return True

    def custom_pol_board_family(self):
        ConfigMgr.instr['polBoardFamily'] = self.varPolBoardFamilyCustom.get()
        return True

    def custom_pol_board_model(self):
        ConfigMgr.instr['polBoardModel'] = self.varPolBoardModelCustom.get()
        return True
